...
const roomPendingCache = new NodeCache();
...
exports.handleAddRoom = (req, res) => {
  ...
  roomModel
    ...
    .then((affectedRow) => {
      ...
      var imgPromise = null,
        ultilitiesPromise = null;
      // Make image list into a dimensional array for bulk insert after
      const imageList =
        createObjectModul.forBulkInsert(imageInfoList || [], insertId) || [];

      const ultilities = requestUltilities
        ? requestUltilities.map((ultility) => {
            return createObjectModul.createObjectWithKeys(
              ultilitiesKey,
              JSON.parse(ultility)
            );
          })
        : [];
      const ultilitiesList =
        createObjectModul.forBulkInsert(ultilities, insertId) || [];

      imgPromise = imageInfoList
        ? imgModel.insertRoomImage(insertId, imageList)
        : null;
      ultilitiesPromise = requestUltilities
        ? ultilityModel.insertRoomUltilities(insertId, ultilitiesList)
        : null;

      Promise.all([imgPromise, ultilitiesPromise])
        .then((value) => {
          res.status(200).json({
            status: 200,
            message: "Ok",
            data: {
              ...
              ultilitiesList:
                (requestUltilities &&
                  requestUltilities.map((ultility) => JSON.parse(ultility))) ||
                null,
            },
          });
        })
    ...
    .then((values) => {
      res.json({
        ...
        data: values.map((value) => {
          return {
            ...createObjectModul.normalizeObjectByKeyPair(
              keyList1,
              keyList2,
              newKeyList,
              value
...
exports.searchRoom = (req, res) => {
  ...
  if (roomSelectAllCache.has("selectAll")) {
    const multipleFilter = new MulitipleFilter(
      filterList.roomListFilter,
...
exports.getPendingRoom = (req, res) => {
  var fullUrl = req.protocol + "://" + req.get("host") + req.originalUrl;
  var q = url.parse(fullUrl, true);
  const params = q.query;
  ...
  if (roomPendingCache.has("selectAll")) {
    const multipleFilter = new MulitipleFilter(
      filterList.roomPendingFilter,
      roomPendingCache.get("selectAll"),
      ","
    );

    res.json({
      message: "Ok",
      data:roomPendingCache.get("selectAll"),
    });
  } else {
    roomModel
      .getPendingRoom()
      .then((result) => {
        ...
        roomPendingCache.set("selectAll", selectAll, 10);
